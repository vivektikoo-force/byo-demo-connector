import {
    testDescriptionMapping, testDescriptions, knowledgeArticles
} from '../common/healthCheckConstants.js';
import messagingConstants from "../common/messagingConstants";
const axios = require('axios');

let lastReadAckEntry;
let lastDeliveredAckEntry;
let convEntryNumber = 0;
let convEntryIds;
const SERVER_URL = "http://localhost:3030";
const READ_ACK_TITLE = "ReadAcknowledgement";
const DELIVERY_ACK_TITLE = "DeliveryAcknowledgement";

export function appendOutboundMessageToChatList(message, originalFileName, fileName, chatList, clientName, outboundTypingStarted) {
    if (originalFileName && fileName) {
        appendAttachmentOutboundMessageToChatList(originalFileName, SERVER_URL + "/uploads/" + fileName, chatList, clientName, outboundTypingStarted);
    }

    let outboundMessageHTMLElem = generateOutboundMessageHTMLElem(message, clientName);

    appendMessageToChatList(outboundMessageHTMLElem, chatList, outboundTypingStarted);
}

export function appendInboundEventToChatList(type, payload, chatList, outboundTypingStarted) {
    ...
    appendMessageToChatList(inboundMessageHTMLElem, chatList, outboundTypingStarted);
    document.getElementById("eventLog").value = JSON.stringify(JSON.parse(payload.string), undefined, 4);
}

export function appendInboundMessageToChatList(message, attachmentName, attachmentUrl, payloadField, previewImageUrl, inputPayload, chatList, sfSubject, outboundTypingStarted) {
    if (attachmentName && attachmentUrl) {
        appendAttachmentInboundMessageToChatList(attachmentName, attachmentUrl, chatList, outboundTypingStarted, sfSubject);
    } else if (previewImageUrl && inputPayload.url) {
        appendLinkPreviewToChatList(previewImageUrl, inputPayload.url, inputPayload.title, chatList, outboundTypingStarted, sfSubject);
    }
    if (!message && !attachmentName && !attachmentUrl && !inputPayload.url) {
        return;
    }

    let inboundMessageHTMLElem = generateInboundMessageHTMLElem(message, sfSubject);
...
export function handleButtonClick(event, chatList, clientName, outboundTypingStarted) {
    ...
    axios({
        ...
        .then((res) => {
            if (res.status === 200) {
                console.log(res);
                appendOutboundMessageToChatList(messageContent, null, null, chatList, clientName, outboundTypingStarted);
            }
        })
        .catch((err) => {
...
export async function handleAckEvent(data) {
    // submit the request to middleware server
    await axios({
        method: "get", url: SERVER_URL + "/convEntryIds"
    }).then((res) => {
...
export function generateTypingIndicator(show, chatList) {
    var typingIndicatorElement = document.getElementById('typingIndicator');
    if (show) {
        if (!typingIndicatorElement) {
            let htmlElement = htmlToElem(`
                <div class="slds-chat-message__body" id="typingIndicator">
...
function getPageTypeFromPath(path) {
    if (path.includes('ccaas')) {
        return 'ccaas';
    } else if (path.includes('ottapp')) {
        return 'ott';
    } else if (path.includes('byob')) {
        return 'byob';
    }
    return 'unknown';
}

function appendAttachmentOutboundMessageToChatList(fileName, fileUrl, chatList, clientName, outboundTypingStarted) {
    let attachmentOutboundMessageHTMLElem = generateAttachmentForOutboundMessageHTMLElem(fileName, fileUrl, clientName);

    appendMessageToChatList(attachmentOutboundMessageHTMLElem, chatList, outboundTypingStarted);
}

function appendMessageToChatList(htmlElem, chatList, outboundTypingStarted) {
    chatList.appendChild(htmlElem);
    chatList.scrollTop = chatList.scrollHeight;
    reloadTypingIndicator(chatList, outboundTypingStarted);
    beep();
}

function reloadTypingIndicator(chatList, outboundTypingStarted) {
    var typingIndicatorElement = document.getElementById('typingIndicator');
    if (typingIndicatorElement) {
        chatList.removeChild(typingIndicatorElement);
    }
...
function generateInboundMessageHTMLElem(message, sfSubject) {
    let now = new Date();
        ...
        <li class="slds-chat-listitem slds-chat-listitem_inbound">
            <div class="slds-chat-message">
                <div class="slds-chat-message__body">
                    <div class="slds-chat-message__text slds-chat-message__text_inbound">
                        <span>${replaceURLsWithHyperLinks(message)}</span>
                    </div>
                    <div class="slds-chat-message__meta" aria-label="said ${sfSubject} at ${dateTime}">
                        ${sfSubject} • ${dateTime}
                    </div>
                </div>
            </div>
...
function generateOutboundMessageHTMLElem(message, clientName) {
    let now = new Date();
    let dateTime = now.toLocaleString();
    let html = `
        <li class="slds-chat-listitem slds-chat-listitem_outbound">
            <div class="slds-chat-message">
                <div class="slds-chat-message__body">
                    ...
                    <div class="slds-chat-message__meta" aria-label="said ${clientName} at ${dateTime}">
                        ${clientName} • ${dateTime}
...
function generateAttachmentForOutboundMessageHTMLElem(fileName, fileUrl, clientName) {
    let now = new Date();
    let dateTime = now.toLocaleString();
    let html = `
        <li class="slds-chat-listitem slds-chat-listitem_outbound">
...
function generateEventMessageHTMLElem(message) {
    let now = new Date();
    let dateTime = now.toLocaleString();
    let html = '<li class="slds-chat-listitem slds-chat-listitem_event">' + '  <div class="slds-chat-event">' + '    <div class="slds-chat-event__body">' + '      <div class="slds-chat-event__agent-message">' + '					<p>' + dateTime + '</p>' + '        <span>' + replaceURLsWithHyperLinks(message) + '        </span>' + '      </div>' + '    </div>' + '  </div>' + '</li>';
...
function appendAttachmentInboundMessageToChatList(fileName, fileUrl, chatList, outboundTypingStarted, sfSubject) {
    let attachmentInboundMessageHTMLElem = generateAttachmentForInboundMessageHTMLElem(fileName, fileUrl, sfSubject);

    appendMessageToChatList(attachmentInboundMessageHTMLElem, chatList, outboundTypingStarted);
}

function appendLinkPreviewToChatList(previewImageUrl, url, title,  chatList, outboundTypingStarted, sfSubject) {
    let linkPreviewInboundMessageHTMLElem = generateLinkPreviewForInboundMessageHTMLElem(previewImageUrl, url, title, sfSubject);

    appendMessageToChatList(linkPreviewInboundMessageHTMLElem, chatList, outboundTypingStarted);
}
...
function generateLinkPreviewForInboundMessageHTMLElem(previewImageUrl, url, title, sfSubject) {
    let now = new Date();
    let dateTime = now.toLocaleString();
    let html = `
        <li class="slds-chat-listitem slds-chat-listitem_inbound">
                ...
                <div class="slds-chat-message__body">
                    <div class="slds-chat-message__text slds-chat-message__text_inbound">
                        <a href="${url}" target="_blank" rel="noopener noreferrer" title="${title}">
                            <div class="chat-link-message__panel chat-link-message__panel_outbound">
                                <div>
                                    <img src="${previewImageUrl}" />
                                </div>
                                <div class="slds-chat-message__meta chat-link-message__content chat-link-message__content_outbound">
                                    <span>${title}</span>
                                    <br/>
                                    <span>${url}</span>
                                </div>
...
function generateAttachmentForInboundMessageHTMLElem(fileName, fileUrl, sfSubject) {
    let now = new Date();
    let dateTime = now.toLocaleString();
    let html = `
        <li class="slds-chat-listitem slds-chat-listitem_inbound">
